...
export const Navbar = () => {
  ...
  return (
    <nav className="z-50 fixed top-4 left-4 right-4 py-3 px-6 
                   bg-white/20 dark:bg-slate-900/50 backdrop-blur-xl 
                   border border-white/20 dark:border-slate-700/30
                   rounded-full shadow-lg shadow-black/5 dark:shadow-white/5
                   transition-all duration-300">
      <div className="absolute left-6 top-1/2 transform -translate-y-1/2 text-2xl font-light">
        ...
        <div className="flex space-x-4 justify-center items-center">
          {[
            ...
            <Link
              key={link.href}
              href={link.href}
              className={`px-3 py-1 rounded-full transition-all ${pathname === link.href
      ...
      <div className="absolute right-6 top-1/2 transform -translate-y-1/2 flex items-center justify-center w-9 h-9">
        <Avatar className="h-9 w-9 rounded-full overflow-hidden ring-2 ring-white/30 dark:ring-slate-700/40 transition-all hover:ring-indigo-400 dark:hover:ring-indigo-500">
          <AvatarImage
            className="rounded-full w-full h-full object-cover"
            src="https://github.com/shadcn.png"
            alt="@shadcn"
          />
          <AvatarFallback className="bg-gradient-to-br from-indigo-500 to-purple-500 text-white">CN</AvatarFallback>
        </Avatar>
      </div>
    </nav>
  )
